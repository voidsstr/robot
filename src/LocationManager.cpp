...
{
    //ctor
    _gpsDriver = new gpsmm("localhost", DEFAULT_GPSD_PORT);
}

void LocationManager::ReadState()
{
    if (_gpsDriver->stream(WATCH_ENABLE|WATCH_JSON) == NULL) {
        ...
        for (;;) {
            ...
            if (!_gpsDriver->waiting(5000000))
              continue;

            if ((newdata = _gpsDriver->read()) == NULL) {
                //cerr << "Read error.\n";
                //return 1;
            } else {
                PrintState(newdata);
...
void LocationManager::PrintState(struct gps_data_t *collect)
{
